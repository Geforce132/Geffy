...
public class CommandRank implements ICommand<MessageEvent, PrivateMessageEvent> {

	public void exeChan(MessageEvent event, String[] args) throws Exception {		
		if(args.length == 1){ 
			event.respond("I rate " + args[0] + " a " + Utils.getRandomNumber(5) + " out of 5.");
		}else if(args.length == 2){
			event.respond("I rate " + args[0] + " a " + Utils.getRandomNumber(Utils.toInt(args[1])) + " out of " + Utils.toInt(args[1]));
		}
	}
